    ...
    {      
        ...
        private async void PopulateLedger(string assetParam)
        {
            ...
            var transactions = await AppShell.server.Operations
                                     .ForAccount(KeyPair.FromAccountId(AppShell.AccountDetails.AccountId))
            ...
            ledger.AddRange(payments.Where(p => p.To.AccountId == AppShell.AccountDetails.AccountId).Select(c => new LedgerItem() { Date = DateTimeOffset.Parse(c.CreatedAt).UtcDateTime, To = c.To.AccountId, From = c.From.AccountId, Deposits = c.Amount }));
            ledger.AddRange(payments.Where(p => p.From.AccountId == AppShell.AccountDetails.AccountId).Select(c => new LedgerItem() { Date = DateTimeOffset.Parse(c.CreatedAt).UtcDateTime, To = c.To.AccountId, From = c.From.AccountId, Withdrawals = c.Amount }));
